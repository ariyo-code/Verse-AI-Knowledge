...
class KnowledgeBase:
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        return {'release': manifest.get('release', {}).get('version'), 'schema': manifest.get('schema_version'), 'api_snapshot': manifest.get('verse_api_version'), 'uefn_compile': 'NOT TESTED unless matching verification evidence exists', 'runtime': 'NOT TESTED unless matching runtime evidence exists', 'multiplayer': 'NOT TESTED unless matching multiplayer evidence exists'}
    ...
    def search(self, query: str, limit: int=12) -> list[dict[str, Any]]:
        ...
        for doc in docs:
            ...
            score += float(trust_scores.get(trust, trust_scores.get('unknown', 0.35))) * float(weights.get('source_trust', 4.0))
            score += float(validation_scores.get(validation, 0.25)) * float(weights.get('validation', 2.5))
            if score > 0:
                rows.append((score, {'path': doc.get('path'), 'title': doc.get('title'), 'source_trust': trust, 'validation': validation, 'content_role': doc.get('content_role', 'data')}))
