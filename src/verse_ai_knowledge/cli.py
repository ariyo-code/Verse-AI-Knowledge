...
def cmd_claim(root, symbol, field, as_json=False) -> int:
    ...
    if as_json:
        ...
        print(f"{r['symbol']} :: {r['field']} -> {r['decision']}")
        print(r['reason'])
        if r.get('evidence_ids'):
            print('Evidence:', ', '.join(r['evidence_ids']))
        if r.get('suggestions'):
            print('Closest known symbols:', ', '.join(r['suggestions']))
...
def version_data(root):
    ...
    idx = root / 'knowledge/api/evidence_graph.json'
    rev = hashlib_file(idx)[:12] if idx.exists() else 'unknown'
...
def cmd_doctor(root, as_json=False):
    ...
    if as_json:
        ...
        [print(f"[{('PASS' if ok else 'FAIL')}] {name}") for name, ok in checks.items()]
...
def explain(root, symbol, as_json=False):
    ...
    if not row:
        data = {'symbol': symbol, 'presence': 'UNKNOWN', 'decision': 'TODO(API VERIFY)'}
        emit(data, as_json)
        return 2
...
def parser():
    p = argparse.ArgumentParser(prog='verse-ai', description='Verse AI Knowledge V25 CLI')
    ...
    cl.add_argument('--field', default='presence', choices=['presence', 'module', 'kind', 'signature', 'parameters', 'return_type', 'effects', 'event_names', 'event_payloads', 'member_names'])
    ...
    err.add_argument('query')
    err.add_argument('--limit', type=int, default=10)
    ...
    ms = maint.add_subparsers(dest='maintenance_cmd', required=True)
    ms.add_parser('check')
    prov = sub.add_parser('provenance')
    prs = prov.add_subparsers(dest='provenance_cmd', required=True)
    s = prs.add_parser('stamp')
    s.add_argument('file')
    s.add_argument('--status', default='draft')
    r = prs.add_parser('read')
    r.add_argument('file')
    ck = prs.add_parser('check')
...
def main(argv=None):
    ...
    kb = KnowledgeBase(root)
    if args.cmd == 'search':
        x = [args.query, '--limit', str(args.limit)] + (['--json'] if args.json else [])
        return run_script(root, 'tools/rag_query.py', x)
    if args.cmd == 'context':
        return run_script(root, 'tools/rag_context_pack.py', [args.query, '--budget', str(args.budget), '--output', args.output])
    if args.cmd == 'api':
        return cmd_api(root, args.symbol, args.json)
    if args.cmd == 'claim':
        return cmd_claim(root, args.symbol, args.field, args.json)
    if args.cmd == 'explain':
        return explain(root, args.symbol, args.json)
    if args.cmd == 'evidence':
        data = kb.get_evidence(args.symbol)
        if data is None:
            emit({'symbol': args.symbol, 'decision': 'TODO(API VERIFY)'}, args.json)
            return 2
        emit(data, args.json)
        return 0
    if args.cmd == 'coverage':
        data = kb.get_coverage()
        emit(data, args.json)
        return 0
    if args.cmd == 'api-queue':
        p = root / 'knowledge/api/verification_queue.json'
        data = json.loads(p.read_text(encoding='utf-8'))
        entries = (data.get('entries') or data.get('queue') or [])[:args.limit]
        emit({'entries': entries, 'count': len(entries)}, args.json)
        return 0
    if args.cmd == 'lint':
        return run_script(root, 'tools/lint_verse_claims.py', [args.file] + (['--json'] if args.json else []) + (['--strict'] if args.strict else []))
    if args.cmd == 'errors':
        return run_script(root, 'tools/search_errors.py', [args.query, '--limit', str(args.limit)] + (['--json'] if args.json else []))
    if args.cmd == 'version':
        emit(version_data(root), args.json)
        return 0
    if args.cmd == 'status':
        emit({'version': version_data(root), 'coverage': kb.get_coverage(), 'uefn_compile': 'NOT TESTED', 'runtime': 'NOT TESTED', 'multiplayer': 'NOT TESTED', 'llm_benchmark': 'SKIPPED unless configured'}, args.json)
        return 0
    if args.cmd == 'snapshot':
        return run_script(root, 'tools/api_coverage_snapshot.py', ['--output', args.output] if args.output else [])
    if args.cmd == 'api-diff':
        return run_script(root, 'tools/api_version_diff.py', [args.old, args.new] + (['--json'] if args.json else []))
    ...
    if args.cmd == 'llm-eval':
        x = []
        if args.responses:
            x += ['--responses', args.responses]
        if args.output:
            x += ['--output', args.output]
        return run_script(root, 'evals/llm/runner.py', x)
    if args.cmd == 'doctor':
        return cmd_doctor(root, args.json)
    if args.cmd == 'validate':
        return run_script(root, 'tools/validate_all.py', [])
    return 2
