...
def repo_root(start: Path | None = None) -> Path:
    ...
    for candidate in (here, *here.parents):
        if (candidate / "manifest.json").exists() and (candidate / "AGENTS.md").exists():
            return candidate
    # Package layout: <repo>/src/verse_ai_knowledge/policy.py
...
def exact_signature_allowed(symbol: dict[str, Any]) -> bool:
    return bool(
        ...
        and symbol.get("exact_signature_claim_allowed")
        and canonical_source_trust(symbol.get("source_trust")) in {"signature-verified", "api-page-verified"}
    )
