...
def resolve_claim(symbol: str, field: str='presence', *, root: Path | None=None) -> dict[str, Any]:
    root = root or repo_root()
    row, suggestions = lookup(symbol, root)
    if not row:
        return {'symbol': symbol, 'field': field, 'decision': 'TODO(API VERIFY)', 'supported': False, 'reason': 'Unknown structured API symbol.', 'evidence_ids': [], 'suggestions': suggestions}
    revalidation = (row.get('revalidation') or {}).get('status', 'unknown')
    if revalidation not in {'current', 'not-applicable'}:
        return {'symbol': row.get('symbol_id'), 'field': field, 'decision': 'TODO(API VERIFY)', 'supported': False, 'reason': f'Stored evidence requires revalidation: {revalidation}.', 'evidence_ids': _evidence_for(row, field), 'suggestions': []}
    ...
    if field == 'presence':
        ...
        return {'symbol': row.get('symbol_id'), 'field': field, 'decision': 'TODO(API VERIFY)', 'supported': False, 'reason': 'Unknown claim field.', 'evidence_ids': [], 'suggestions': []}
    return {'symbol': row.get('symbol_id'), 'field': field, 'decision': 'ALLOW' if ok else 'TODO(API VERIFY)', 'supported': bool(ok), 'reason': 'Field-level evidence is available.' if ok else 'Field-level evidence is insufficient.', 'evidence_ids': evidence_ids, 'source_url': row.get('source_url'), 'api_version': row.get('api_version'), 'claim_scope': row.get('claim_scope'), 'suggestions': []}
